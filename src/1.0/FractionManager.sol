...
contract FractionManager is Context, ERC165Storage, ERC721A__IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    constructor(address _wsnsr, address usdc, address _feed) {
        ...
        _usdPrice = 100 * 10 ** _priceFeed.decimals(); // 100 USD by default
        // mint all ERC20 tokens
        totalFractions = 5000 * FRACTION_SIZE * 10 ** _priceFeed.decimals();
        wsnsr.setMaxSupply(totalFractions);
    ...
    function redeemFraction(uint256 _tokenId, uint256 _amount) external nonReentrant {
        ...
        require(_amount <= _availableFractions[_tokenId], "FractionManager: insufficient available fractions");
        require(
            _fractionOwnership[address(this)][_tokenId] > 0,
            "Sender do not own any fraction of the token"
        );
    ...
    function sendFraction(address _to, uint256 _tokenId, uint256 _amount) external onlyOwner {
        ...
        require(_tokenId >= 3 && _tokenId < 5001, "FractionManager: invalid tokenId");
        require(_amount <= _availableFractions[_tokenId], "FractionManager: insufficient available fractions");
        _transferOut(_tokenId, _amount, msg.sender, _to);
    }

    function transferFraction(uint256 _tokenId, uint256 _amount, address _from, address _to) external nonReentrant returns (bool) {
        require(
            wsnsr.transferByManager(_tokenId, _amount, _from, _to),
            "Fraction Transfer Failure"
        );
        require(
            _fractionOwnership[_from][_tokenId] > 0,
            "Sender do not own any fraction of the token"
        );
    ...
    function developerCheck(uint256 _amount) private {
        ...
        } else if ((totalFractionSold >= breakpoint1) && (_fractionOwnership[devWallet][2] < 1000)) {
            totalFractionSold += 1000;
            _fractionOwnership[devWallet][2] += 1000;
        } else if ((totalFractionSold >= breakpoint1) && (_fractionOwnership[devWallet][1] < 1000)) {
            totalFractionSold += 1000;
        ...
        returns (bool)
    {
        return
            ...
            interfaceId == type(ERC721A__IERC721Receiver).interfaceId ||
    ...
    function onERC721Received(
        ...
        _availableFractions[tokenId] = FRACTION_SIZE;
        require(
            wsnsr.transferByManager(tokenId, FRACTION_SIZE, address(wsnsr), address(this)),
            "Fraction Transfer Failure"
        );
        _fractionOwnership[address(this)][tokenId] += FRACTION_SIZE;
